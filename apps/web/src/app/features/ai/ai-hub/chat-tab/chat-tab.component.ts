          ...
          @if (messages().length === 0) {
            <div class="empty-state">
              ...
              <div class="quick-actions">
                <button class="quick-action" (click)="setInput(i18n.t().aiHub.quickPrompts.greeting)">
                ...
                <button class="quick-action" (click)="setInput(i18n.t().aiHub.quickPrompts.creative)">
...
export class ChatTabComponent implements OnInit, OnDestroy {
  ...
  messages = signal<{ id: string; role: 'user' | 'assistant'; content: string; timestamp: number }[]>([]);
  input = signal('');
  ...
  send() {
    ...
    const userMsg = { id: `user_${Date.now()}`, role: 'user' as const, content: this.input().trim(), timestamp: Date.now() };
    ...
    this.messages.update((msgs) => [...msgs, { id: assistantId, role: 'assistant', content: '', timestamp: Date.now() }]);
    ...
    this.api.chatStream(
      { messages: history, session_id: this.sessionId, provider: this.selectedProvider(), model: this.selectedModel() },
      (chunk) => {
        fullContent += chunk;
