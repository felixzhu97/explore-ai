...
/**
 * Composite tool: model evaluation using RAG retrieval quality and JVM metrics.
 */
@Component
public class ModelEvaluationTool implements ToolProvider {

    private final ToolRegistryPort registry;
    private final CompositeToolSupport support;

    public ModelEvaluationTool(@Lazy ToolRegistryPort registry) {
    ...
    public ToolDefinition definition() {
        ...
        return ToolDefinition.composite(
            "model_evaluation",
            "Model evaluation: searches for LLM evaluation metrics and retrieves JVM metrics.",
    ...
    public ToolResult execute(ToolInvocation invocation) {
        try {
            ...
            List<SubToolCall> calls = List.of(
                new SubToolCall("rag_search", Map.of("query", "LLM evaluation metrics", "topK", topK)),
                new SubToolCall("get_jvm", Map.of())
            );

            CompositeResult result = support.executeAll(calls);

            String md = "# Model Evaluation\n\n" + result.markdown();
            return ToolResult.success(md, result.structured());
        } catch (Exception e) {
            return ToolResult.error("model_evaluation failed: " + e.getMessage());
        }
    }
}
