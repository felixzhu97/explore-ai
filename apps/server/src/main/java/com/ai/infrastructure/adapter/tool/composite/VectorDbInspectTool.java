...
/**
 * Composite tool: vector database inspection.
 */
@Component
public class VectorDbInspectTool implements ToolProvider {

    private final ToolRegistryPort registry;
    private final CompositeToolSupport support;

    public VectorDbInspectTool(@Lazy ToolRegistryPort registry) {
    ...
    public ToolDefinition definition() {
        ...
        return ToolDefinition.composite(
            "vectordb_inspect",
            "Vector database inspection: lists all documents and searches for vector database best practices.",
    ...
    public ToolResult execute(ToolInvocation invocation) {
        try {
            ...
            List<SubToolCall> calls = List.of(
                new SubToolCall("document_list", Map.of("limit", 50)),
                new SubToolCall("rag_search", Map.of("query", "vector database", "topK", topK))
            );

            CompositeResult result = support.executeAll(calls);

            String md = "# VectorDB Inspection\n\n" + result.markdown();
            return ToolResult.success(md, result.structured());
        } catch (Exception e) {
            return ToolResult.error("vectordb_inspect failed: " + e.getMessage());
        }
    }
}
