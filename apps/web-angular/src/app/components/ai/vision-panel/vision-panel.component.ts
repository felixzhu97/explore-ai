...
@Component({
        ...
        <div class="panel">
          ...
          <div class="result-content">
            @if (currentState().result) {
              ...
              <div class="empty-state">{{ i18n.t().imageUploader.noImageYet }}</div>
            }
          </div>
          ...
          <div class="action-area">
            ...
            >
              @if (isLoading()) {
                <span class="btn-spinner"></span> {{ i18n.t().imageUploader.analyzing }}
              } @else {
  ...
  styles: [`
    ...
    .image-area:hover { background: rgba(0, 0, 0, 0.02); }
    ...
    .image-area:hover .clear-button { opacity: 1; }
    .clear-button:hover { background: rgba(0, 0, 0, 0.8); }
...
export class VisionPanelComponent {
  ...
  submit() {
    ...
    request.subscribe({
      ...
      error: (err: unknown) => {
        const msg = err instanceof Error ? err.message : this.i18n.t().imageUploader.processingFailed;
