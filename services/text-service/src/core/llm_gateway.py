"""Unified LLM Gateway for Text-to-Text service.
...
class ChatMessage(Dict[str, str]):
    """Chat message format matching the API schema."""
...
class LLMGateway:
    ...
    def get_llm(
        ...
        if provider == "openai":
            ...
            model = model or settings.ANTHROPIC_MODELS.split(",")[0]
...
class TextToTextService:
    ...
    def __init__(
        ...
        """Initialize the text-to-text service.
    ...
    def chat(
        self,
        messages: List[Union[str, Dict[str, str], ChatMessage]],
        system_prompt: Optional[str] = None,
        **kwargs,
    ) -> str:
        ...
        for msg in messages:
            if isinstance(msg, str):
                ...
                # Message object with role attribute
...
def get_text_service(
    ...
    """Create a TextToTextService instance.
    ...
    Returns:
        Configured TextToTextService instance
    """
    return TextToTextService(
        provider=provider,
        model=model,
        temperature=temperature,
