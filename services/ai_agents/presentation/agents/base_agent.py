...
class BaseInfraAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            max_message_window: Maximum number of messages to keep in context.
            message_strategy: Strategy for message windowing ("sliding" or "truncate").
        """
        self.llm = llm
        self.tools = tools or []
    ...
    async def astream(self, input_data: Dict[str, Any]):
        ...
        if len(messages) > self.max_message_window:
            if self.message_strategy == "sliding":
                messages = messages[-self.max_message_window:]
            else:
                half = self.max_message_window // 2
                messages = messages[:half] + messages[-half:]
            logger.info(f"Message window applied: {len(input_data['messages'])} -> {len(messages)}")
