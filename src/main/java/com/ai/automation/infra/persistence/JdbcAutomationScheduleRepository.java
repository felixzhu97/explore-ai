...
public class JdbcAutomationScheduleRepository implements AutomationScheduleRepository {
  ...
  private final RowMapper<AutomationSchedule> rowMapper =
      (rs, rowNum) ->
    ...
    jdbcTemplate.update(
        """
                MERGE INTO automation_schedules (
                    id, owner_key, name, schedule_kind, cron_expression, timezone,
                    enabled, action_type,
  ...
  public void deleteByIdAndClientId(ScheduleId id, String clientId) {
    jdbcTemplate.update(
        "DELETE FROM automation_schedules WHERE id = ? AND owner_key = ?", id.value(), clientId);
  }
