...
@ConditionalOnProperty(prefix = "app.mail", name = "provider", havingValue = "resend", matchIfMissing = true)
...
public class ResendEmailGateway implements EmailGateway {
    ...
    public ResendEmailGateway(MailProperties mailProperties) {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException(
                    "app.mail.resend-api-key (APP_MAIL_RESEND_API_KEY) is required when app.mail.provider=resend");
        }
    ...
    public void send(EmailMessage message) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Resend API rejected email: HTTP " + e.getStatusCode().value() + " " + e.getResponseBodyAsString(),
                    e);
