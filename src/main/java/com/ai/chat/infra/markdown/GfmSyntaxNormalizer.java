package com.ai.chat.infra.markdown;
...
/**
 * Repairs common GFM syntax spacing issues (e.g. "-item" → "- item") without changing content
 * semantics. See GitHub Flavored Markdown spec for list and heading token rules.
 */
@Component
public class GfmSyntaxNormalizer {
  ...
  public String normalize(String content) {
    ...
    while (hrMatcher.find()) {
      ...
      hrMatcher.appendReplacement(protectedContent, HR_PLACEHOLDER + (horizontalRules.size() - 1));
    }
    ...
    normalized =
        normalized.replaceAll(
            "([：。！？；:」』\"'])(-{3,}|_{3,}|\\*{3,})(#{1,6})(?=[^\\s#\\n])", "$1\n$2\n$3 ");
    normalized = normalized.replaceAll("(-{3,}|_{3,}|\\*{3,})(#{1,6})(?=[^\\s#\\n])", "$1\n$2 ");
    ...
    normalized =
        normalized.replaceAll("([一二三四五六七八九十]+、[^\\n-]+)(-(?!-)(?=[^\\s\\n0-9a-zA-Z]))", "$1\n$2");
    ...
    for (int index = 0; index < codeBlocks.size(); index++) {
      normalized =
          normalized.replace(
              CODE_PLACEHOLDER_PREFIX + index + CODE_PLACEHOLDER_SUFFIX, codeBlocks.get(index));
    }
