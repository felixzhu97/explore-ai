...
import com.ai.eval.domain.CaseEvalOutcome;
...
/**
 * Runs OpenAI Evals-style golden cases against live Chat / RAG generation,
 * then scores with Spring AI official evaluators. Intended for tests (no REST).
 */
@Service
public class GoldenEvalUseCase {
    ...
    public GoldenSuiteReport run(List<GoldenEvalDomain> domains, List<String> caseIds) {
        ...
        Map<String, String> fixtureIds = cases.stream().anyMatch(c -> c.domain() == GoldenEvalDomain.RAG)
    ...
    private CaseEvalOutcome runOne(GoldenEvalCase evalCase, Map<String, String> fixtureIds) {
        try {
            ...
            OfficialGateResult gate = officialEvaluators.evaluate(
                    evalCase.userText(), generated.answer(), context);
    ...
    private GeneratedAnswer generate(GoldenEvalCase evalCase, Map<String, String> fixtureIds) {
        if (evalCase.domain() == GoldenEvalDomain.RAG) {
            ...
            List<String> sources = result.sources() == null
                    ? List.of()
        ...
        TextChatOptions options = evalCase.toolsEnabled()
    ...
    private static List<String> resolveDocumentIds(GoldenEvalCase evalCase, Map<String, String> fixtureIds) {
