...
import com.ai.metrics.application.DrilldownPage;
import com.ai.metrics.application.MetricsDomainSnapshot;
import com.ai.metrics.application.MetricsOverview;
import com.ai.metrics.application.NamedCount;
import com.ai.metrics.application.SeriesPoint;
import com.ai.metrics.application.SeriesSnapshot;
...
public class MetricsUseCase {
    ...
    public MetricsOverview overview(String range) {
        ...
        var latency = queryRepository.latencyPercentiles(Optional.empty(), window.from(), window.to());
        var tokens = queryRepository.tokenTotals(Optional.empty(), window.from(), window.to());
        ...
        List<NamedCount> byDomain = queryRepository.countByDomain(window.from(), window.to()).stream()
                .map(nc -> new NamedCount(nc.name(), nc.count()))
                .toList();

        Map<String, Object> domains = new LinkedHashMap<>();
        domains.put("chat", Map.of(
    ...
    public MetricsDomainSnapshot domain(String domainRaw, String range) {
        ...
        Map<String, Object> inventory = switch (domain) {
            case CHAT -> {
    ...
    public SeriesSnapshot series(String name, String domainRaw, String range) {
        ...
        List<SeriesPoint> points = switch (seriesName) {
            case "requests" -> toPoints(queryRepository.dailyRequests(domain, window.from(), window.to()));
            case "errors" -> toPoints(queryRepository.dailyErrors(domain, window.from(), window.to()));
            case "latency_p95" -> toPoints(queryRepository.dailyLatencyP95(domain, window.from(), window.to()));
            case "sessions_created" -> toPoints(queryRepository.dailySessionsCreated(window.from(), window.to()));
            case "messages_created" -> toPoints(queryRepository.dailyMessagesCreated(window.from(), window.to()));
            case "documents_uploaded" -> toPoints(queryRepository.dailyDocumentsUploaded(window.from(), window.to()));
            ...
            case "calls_by_model" -> queryRepository.countByModel(domain, window.from(), window.to()).stream()
                    .map(nc -> new SeriesPoint(nc.name(), nc.count()))
                    .toList();
            case "calls_by_agent" -> queryRepository.countByAgentType(window.from(), window.to()).stream()
                    .map(nc -> new SeriesPoint(nc.name(), nc.count()))
                    .toList();
            case "tool_top" -> queryRepository.topTools(domain, window.from(), window.to(), 10).stream()
                    .map(nc -> new SeriesPoint(nc.name(), nc.count()))
                    .toList();
            case "tokens" -> {
        ...
        return new SeriesSnapshot(seriesName, domain.map(AiDomain::value).orElse(null), window.range(), points);
    }
    ...
    public DrilldownPage drilldown(
        ...
        var result = eventRepository.findDrilldown(new AiInvocationEventRepository.DrilldownQuery(
