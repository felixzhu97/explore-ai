...
public class ChatModelResolver {
  ...
  private ResolvedChatModel resolveOllama(String model) {
    ...
    if (ollama == null) {
      throw new IllegalArgumentException(
          "Ollama chat model is not configured. Enable spring.ai.ollama.chat.enabled"
              + " and ensure Ollama is reachable.");
    }
    String effectiveModel = model == null || model.isBlank() ? defaultOllamaModel : model;
    ChatOptions.Builder<?> chatOptions = OllamaChatOptions.builder().model(effectiveModel);
