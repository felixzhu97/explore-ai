...
/**
 * Strips DeepSeek DSML tool-call markup from model text. Matches any angle-bracket tag containing
 * {@code DSML}, including fullwidth {@code ｜} and oddly spaced variants.
 *
 * @see <a
 *     href="https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/blob/main/encoding/README.md">DeepSeek
 *     DSML</a>
...
public final class ToolCallMarkupFilter {
  ...
  private static final Pattern DSML_PAIR_BLOCK =
      Pattern.compile(
          "<\\s*[^<>]*DSML[^<>]*>[\\s\\S]*?</\\s*[^<>]*DSML[^<>]*>", Pattern.CASE_INSENSITIVE);

  private static final Pattern DSML_UNCLOSED =
      Pattern.compile("<\\s*[^<>]*DSML[^<>]*>[\\s\\S]*$", Pattern.CASE_INSENSITIVE);

  private static final Pattern DSML_ANY_TAG =
      Pattern.compile("</?\\s*[^<>]*DSML[^<>]*>", Pattern.CASE_INSENSITIVE);
