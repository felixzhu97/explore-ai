...
public class AudioController {
    ...
    public AudioController(TextToSpeechUseCase textToSpeechUseCase) {
    ...
    public ResponseEntity<byte[]> speak(@RequestBody TtsRequest request) {
        try {
            byte[] audio = textToSpeechUseCase.synthesize(request.text());
    ...
    public ResponseEntity<Flux<byte[]>> stream(@RequestParam String text) {
        try {
            Flux<byte[]> audioStream = textToSpeechUseCase.stream(text);
    ...
    public ResponseEntity<Map<String, Object>> getVoices() {
        return ResponseEntity.ok(Map.of(
                "voices", textToSpeechUseCase.getAvailableVoices()
        ));
    }
    ...
    public ResponseEntity<Map<String, Object>> getModels() {
        return ResponseEntity.ok(Map.of(
                "models", textToSpeechUseCase.getAvailableModels()
        ));
    }
