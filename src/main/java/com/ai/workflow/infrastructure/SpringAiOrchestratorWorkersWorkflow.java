...
/**
 * Orchestrator-workers aligned with Spring AI agentic-patterns/orchestrator-workers, with parallel
 * workers via CompletableFuture and a synthesizer step.
 */
@Component
public class SpringAiOrchestratorWorkersWorkflow implements OrchestratorWorkersWorkflow {
  ...
  static final String DEFAULT_ORCHESTRATOR_PROMPT =
      """
            ...
            {
            "analysis": "Explain your understanding of the task and which variations would be
            valuable. Focus on how each approach serves different aspects of the task.",
            "tasks": [
                {
                "type": "formal",
  ...
  public OrchestratorWorkersResult process(String task) {
    ...
    List<WorkerTask> tasks =
        plan.tasks().stream().map(t -> new WorkerTask(t.type(), t.description())).toList();
    ...
    return new OrchestratorWorkersResult(
        plan.analysis(), tasks, workerResponses, synthesis == null ? "" : synthesis);
  }
