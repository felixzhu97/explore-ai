...
describe('AgentsPage httpResource', () => {
  ...
  it('should_load_agents_via_httpResource_when_mounted', async () => {
    const agents = runInInjectionContext(injector, () => {
      const resource = httpResource<AgentInfo[]>(() => `${API_BASE_URL}/agents/list`);
    ...
    TestBed.tick();
    const req = http.expectOne(`${API_BASE_URL}/agents/list`);
    req.flush([
      {
        type: 'supervisor',
        name: 'Supervisor',
  ...
  it('should_exposeError_when_agentsRequestFails', async () => {
    const resource = runInInjectionContext(injector, () => httpResource<AgentInfo[]>(() => `${API_BASE_URL}/agents/list`),
    );

    TestBed.tick();
    http.expectOne(`${API_BASE_URL}/agents/list`).error(new ProgressEvent('error'));
    await TestBed.inject(ApplicationRef).whenStable();
