...
import { ZardIdDirective, ZardStringTemplateOutletDirective } from '../../zard';
    ...
    <ng-container zardId="card" #z="zardId">
      ...
      @if (title) {
        <div [class]="headerClasses()" data-slot="card-header">
          ...
          @if (description) {
            <div class="text-sm text-muted-foreground" [id]="descriptionId()" data-slot="card-description">
...
export class ZardCardComponent {
  ...
  protected readonly footerClasses = computed(() => mergeClasses(cardFooterVariants(), this.zFooterBorder() ? 'border-t' : ''),
  );

  protected readonly headerClasses = computed(() => mergeClasses(cardHeaderVariants(), this.zHeaderBorder() ? 'border-b' : ''),
  );
