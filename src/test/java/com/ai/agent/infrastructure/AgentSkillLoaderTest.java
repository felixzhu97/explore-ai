...
import com.ai.agent.domain.AgentSkill;
import com.ai.agent.infrastructure.AgentProperties;
...
class AgentSkillLoaderTest {
    ...
    private static AgentSkillLoader loader(boolean enabled, List<String> ids) {
        AgentProperties p = new AgentProperties(); p.getSkills().setEnabled(enabled); p.getSkills().setIds(ids); return new AgentSkillLoader(p);
    }
}
