...
class VisionControllerTest {
    ...
    class Caption {
        ...
        void should_return_caption_response() throws Exception {
            ...
            ResponseEntity<CaptionResponse> response = controller.caption(file);

            assertThat(response.getStatusCode().value()).isEqualTo(200);
            assertThat(response.getBody().caption()).isEqualTo("A cat on a sofa");
        }

        @Test
        @DisplayName("should return 400 for empty file")
        void should_return_400_for_empty_file() {
            ResponseEntity<CaptionResponse> response = controller.caption(null);
    ...
    class Detect {
        ...
        void should_return_detections() throws Exception {
            ...
            ResponseEntity<DetectResponse> response = controller.detect(file);

            assertThat(response.getStatusCode().value()).isEqualTo(200);
            assertThat(response.getBody().detections()).hasSize(1);
        }
    }
